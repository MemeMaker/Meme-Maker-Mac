...
NSString *kPrivateDragUTI = @"com.avikantz.cocoadraganddrop";
...
- (void)dragImage:(NSImage *)anImage at:(NSPoint)viewLocation offset:(NSSize)initialOffset event:(NSEvent *)event pasteboard:(NSPasteboard *)pboard source:(id)sourceObj slideBack:(BOOL)slideFlag {
	...
	// Get the drag size
	NSSize dragSize = NSSizeFromCGSize(CGSizeMake([self bounds].size.width*0.8, (0.8*[self bounds].size.width)*(dragImage.size.height/dragImage.size.width)));
	[dragImage setSize:dragSize];
...
- (NSArray *)namesOfPromisedFilesDroppedAtDestination:(NSURL *)dropDestination {
	...
	if ([[[representations objectAtIndex:0] className] isEqualToString:@"NSBitmapImageRep"]) {
		bitmapData = [NSBitmapImageRep representationOfImageRepsInArray:representations
															  usingType:NSJPEGFileType properties:@{}];
	}
	else {
		...
		// Cause we want to save a jpg
		bitmapData = [imageRep representationUsingType:NSJPEGFileType properties:imageProps];
	}
